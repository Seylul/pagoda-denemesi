...
function wpmu_current_site() {
	...
	// Still no dice.
	if ( 1 == count( $sites ) )
		wp_die( sprintf( /*WP_I18N_BLOG_DOESNT_EXIST*/'Böyle bir site yok. Lütfen <a href="%s">%s</a> adresini deneyin.'/*/WP_I18N_BLOG_DOESNT_EXIST*/, $sites[0]->domain . $sites[0]->path ) );
	else
		wp_die( /*WP_I18N_NO_SITE_DEFINED*/'Bu sunucu için site tanımlanmamış. Eğer bu sitenin sahibiyseniz, lütfen <a href="http://codex.wordpress.org/Debugging_a_WordPress_Network">WordPress ağları bakım ve onarım</a> belgesini inceleyin.'/*/WP_I18N_NO_SITE_DEFINED*/ );
}
...
function ms_not_installed() {
	...
	$title = /*WP_I18N_FATAL_ERROR*/'Veritabanı bağlantısı kurulurken hata oluştu'/*/WP_I18N_FATAL_ERROR*/;
	$msg  = '<h1>' . $title . '</h1>';
	if ( ! is_admin() )
		die( $msg );
	$msg .= '<p>' . /*WP_I18N_CONTACT_OWNER*/'Eğer siteniz görüntülenemiyorsa lütfen bu ağın sahibi ile iletişim kurun.'/*/WP_I18N_CONTACT_OWNER*/ . '';
	$msg .= ' ' . /*WP_I18N_CHECK_MYSQL*/'Eğer bu ağın yöneticisi iseniz lütfen MySQL sunucusunun çalışır durumda olduğundan ve tüm tabloların hatasız olduğundan emin olun.'/*/WP_I18N_CHECK_MYSQL*/ . '</p>';
	if ( false && !$wpdb->get_var( "SHOW TABLES LIKE '$wpdb->site'" ) )
		$msg .= '<p>' . sprintf( /*WP_I18N_TABLES_MISSING_LONG*/'<strong>Veritabanı tabloları eksik.</strong> MySQL çalışmıyor, WordPress düzgün yüklenmemiş ya da birileri <code>%s</code> öğelerini silmiş olabilir. Hemen veritabanınızı kontrol etmelisiniz.'/*/WP_I18N_TABLES_MISSING_LONG*/, $wpdb->site ) . '</p>';
	else
		$msg .= '<p>' . sprintf( /*WP_I18N_NO_SITE_FOUND*/'<strong>Site bulunamıyor <code>%1$s</strong>. <code>%3$s</code> içinde yer alan <code>%2$s</code> tablosunda arandı. Doğru mudur?'/*/WP_I18N_NO_SITE_FOUND*/, rtrim( $domain . $path, '/' ), $wpdb->blogs, DB_NAME ) . '</p>';
	$msg .= '<p><strong>' . /*WP_I18N_WHAT_DO_I_DO*/'What do I do now?'/*WP_I18N_WHAT_DO_I_DO*/ . '</strong> ';
	$msg .= /*WP_I18N_RTFM*/'<a target="_blank" href="http://codex.wordpress.org/Debugging_a_WordPress_Network">Hata bildirimi</a> sayfasını okuyun. Bazı bilgiler neyin ters gittiğini anlamanızda yardımcı olabilir.'/*/WP_I18N_RTFM*/;
	$msg .= ' ' . /*WP_I18N_STUCK*/'Eğer bu mesaja takılıp kaldıysanız, o zaman veritabanında aşağıdaki tablolar var mı kontrol edin:'/*/WP_I18N_STUCK*/ . '</p><ul>';
